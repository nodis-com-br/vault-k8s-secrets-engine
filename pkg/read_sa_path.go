...
const keyRoleBindingScope = "scope"
	...
	return &framework.Path{
		...
		Fields: map[string]*framework.FieldSchema{
			...
			keyRoleBindingScope: &framework.FieldSchema{
				Type:        framework.TypeString,
				Description: "Role binding scope, cluster or namespace. If not set wil be namespace.",
			},
		},
...
func (b *backend) handleReadForRole(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	if d != nil {
		...
		if scope != "cluster" && scope != "namespace" {
			return nil, fmt.Errorf("'%s' in an invalid value for %s", scope, keyRoleBindingScope)
		}

		ttl := d.Get(keyTTLSeconds).(int)
		return b.createSecret(ctx, req.Storage, saType, namespace, ttl, scope)
	}
