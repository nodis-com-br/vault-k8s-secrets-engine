...
func (b *backend) createSecret(ctx context.Context, s logical.Storage, saType string, namespace string, ttl int, scope string) (*logical.Response, error) {
	...
	resp := b.Secret(secretAccessKeyType).Response(map[string]interface{}{
		...
		keyRoleBindingName:     bindingName,
		keyRoleBindingScope:    scope,
...
func (b *backend) revokeSecret(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	scope := d.Get(keyRoleBindingScope).(string)

	if scope == "cluster" {
		b.Logger().Info(fmt.Sprintf("deleting cluster role binding with name: %s", roleBindingName))
		err = b.kubernetesService.DeleteClusterRoleBinding(pluginConfig, roleBindingName)
		if err != nil {
			return nil, err
		}
		b.Logger().Info(fmt.Sprintf("deleted cluster role binding with name: %s", roleBindingName))
	} else {
...
// getClusterRoleName is a helper function to pull out the correct cluster role from the plugin configuration for the saType
